...
// Trata os dados relacionados aos funcionarios
...
public class DaoFuncionario extends Conexao {
	...
	// abrir conexao para tratar funcionarios utilizando o recurso extends
	// aproveitando os dados da classe de conexao
	public Vector<Funcionario> funcionarios() {
		...
		try {
			...
			String sql = "SELECT F.ID_FUNC, U.NOME_UNIDADE, F.NOME_FUNC, F.CPF_FUNC, F.RG_FUNC, F.TELEFONE_FUNC, F.HORA_INICIO_FUNC, F.HORA_FIM_FUNC "
					+ "FROM TB_FUNCIONARIO F " + "INNER JOIN TB_UNIDADE U ON U.ID_UNIDADE = F.ID_UNIDADE ";
	...
	// Traz os dados da tabela de funcionarios
	public void preencherTabela(Vector<Funcionario> vetorFunc, JTable tabelaFunc) {
		...
		for (int i = 0; i < vetorFunc.size(); i++) {
			...
			model.addRow(new String[] { funcionario.getIdFunc(), funcionario.getNomeFunc(), funcionario.getCpf(),
					funcionario.getRg(), funcionario.getTelefone(), funcionario.getIdUnidade(),
